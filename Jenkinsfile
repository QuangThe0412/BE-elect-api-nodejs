            ...
            steps {
                script {
                    sshagent(['ssh-smart-shop']) {
                        ...
                        sh 'ssh -o StrictHostKeyChecking=no -p 666 -i ~/.ssh/id_rsa quangthe@127.0.0.1 "cd /home/smart-shop && unzip -o build.zip && cd build && pm2 start ecosystem.config.js --env production"'
                    }
                }
            }
        }
